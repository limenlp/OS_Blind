...
from mm_agents.mirrorguard import MirrorGuardUITARSAgent
import os
...
def config() -> argparse.Namespace:
    ...
    # MirrorGuard config
    parser.add_argument("--mirrorguard_model", type=str, default="WhitzardAgent/MirrorGuard",
                        help="MirrorGuard model name (served via vLLM)")
    parser.add_argument("--mirrorguard_api_url", type=str, default=None,
                        help="MirrorGuard API URL (default: env MIRRORGUARD_API_URL or http://localhost:8003/v1)")
    parser.add_argument("--mirrorguard_api_key", type=str, default=None,
                        help="MirrorGuard API key (default: env MIRRORGUARD_API_KEY or 'dummy')")
    parser.add_argument("--mirrorguard_max_tokens", type=int, default=2048,
                        help="Max tokens for MirrorGuard corrector output")
    parser.add_argument("--mirrorguard_temperature", type=float, default=0.0,
    ...
    # logging related
    parser.add_argument("--result_dir", type=str, default="./results_uitars15_v1_mirrorguard_round1")
...
def run_env_tasks(task_queue: Queue, args: argparse.Namespace, shared_scores: list):
    ...
    try:
        ...
        # Resolve MirrorGuard config from args or environment variables
        ...
        while True:
            ...
            try:
                ...
                # Use MirrorGuard-enhanced UITARS agent
                agent = MirrorGuardUITARSAgent(
                    # MirrorGuard-specific params
                ...
                try:
                    ...
                    with open(os.path.join(example_result_dir, "traj.jsonl"), "a") as f:
                        ...
                        f.write("\n")
                # Always save MirrorGuard log after each task (success or failure)
                if agent and hasattr(agent, 'mirrorguard_log') and agent.mirrorguard_log:
                    ...
                    try:
                        ...
                        logger.info(f"[MirrorGuard] Saved correction log ({len(agent.mirrorguard_log)} steps) to {mg_log_path}")
