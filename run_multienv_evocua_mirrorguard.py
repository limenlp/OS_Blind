"""
    Script to run EvoCUA + MirrorGuard on OSWorld tasks.
    ...
    export EVOCUA_API_KEY="xxxx"
    export EVOCUA_BASE_URL="xxxx"

    Example Usage (S2):
        python3 run_multienv_evocua_mirrorguard.py \
            --headless \
            --provider_name aws \
...
from mm_agents.mirrorguard_evocua_agent import MirrorGuardEvoCUAAgent
...
load_env(__file__, "evocua")
# Also load MirrorGuard vars if present (without overriding existing vars)
load_env(__file__, "mirrorguard")

def config() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run end-to-end evaluation with MirrorGuardEvoCUAAgent"
    )
    ...
    # lm config
    #parser.add_argument("--model", type=str, default="meituan/EvoCUA-32B-20260105", help="Model name.")
    parser.add_argument("--model", type=str, default="meituan/EvoCUA-8B-20260105", help="Model name.")
    ...
    parser.add_argument(
        "--mirrorguard_model",
        type=str,
        default="WhitzardAgent/MirrorGuard",
        help="MirrorGuard model name (served via OpenAI-compatible endpoint)",
    )
    parser.add_argument(
        "--mirrorguard_api_url",
        type=str,
        default=None,
        help="MirrorGuard API URL (default: env MIRRORGUARD_API_URL or http://localhost:8003/v1)",
    )
    parser.add_argument(
        "--mirrorguard_api_key",
        type=str,
        default=None,
        help="MirrorGuard API key (default: env MIRRORGUARD_API_KEY or 'dummy')",
    )
    parser.add_argument(
        "--mirrorguard_max_tokens",
        type=int,
        default=8192,
        help="Max tokens for MirrorGuard corrected thought",
    )
    parser.add_argument(
        "--mirrorguard_temperature",
        type=float,
        default=0.0,
        help="Temperature for MirrorGuard corrector",
    )
    ...
    # logging related
    parser.add_argument("--result_dir", type=str, default="./results_evocua_mirrorguard_round1")
...
def run_env_tasks(task_queue: Queue, args: argparse.Namespace, shared_scores: list):
    ...
    try:
        ...
        mg_api_url = args.mirrorguard_api_url or os.environ.get(
            "MIRRORGUARD_API_URL", "http://localhost:8003/v1"
        )
        mg_api_key = args.mirrorguard_api_key or os.environ.get(
            "MIRRORGUARD_API_KEY", "dummy"
        )
        mg_model = os.environ.get("MIRRORGUARD_MODEL", args.mirrorguard_model)
        ...
        while True:
            ...
            try:
                ...
                # Initialize MirrorGuard-enhanced EvoCUA agent
                agent = MirrorGuardEvoCUAAgent(
                    mirrorguard_model=mg_model,
                    mirrorguard_api_url=mg_api_url,
                    mirrorguard_api_key=mg_api_key,
                    mirrorguard_max_tokens=args.mirrorguard_max_tokens,
                    mirrorguard_temperature=args.mirrorguard_temperature,
                    model=args.model,
                ...
                try:
                    ...
                    with open(os.path.join(example_result_dir, "traj.jsonl"), "a") as f:
                        ...
                        f.write("\n")
                if agent and hasattr(agent, "mirrorguard_log") and agent.mirrorguard_log:
                    mg_log_path = os.path.join(example_result_dir, "mirrorguard_log.json")
                    try:
                        with open(mg_log_path, "w", encoding="utf-8") as mg_f:
                            json.dump(agent.mirrorguard_log, mg_f, indent=2, ensure_ascii=False)
                        logger.info(
                            f"[MirrorGuard] Saved correction log ({len(agent.mirrorguard_log)} steps) to {mg_log_path}"
                        )
                    except Exception as mg_e:
                        logger.error(f"[MirrorGuard] Failed to save log: {mg_e}")
            except Exception as e:
