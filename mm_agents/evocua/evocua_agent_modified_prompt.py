...
S1_SYSTEM_PROMPT = S1_SYSTEM_PROMPT + "\n\n" + LLM_SAFETY_PROMPT_TEMPLATE
S2_SYSTEM_PROMPT = S2_SYSTEM_PROMPT + "\n\n" + LLM_SAFETY_PROMPT_TEMPLATE

class EvoCUAModifiedPromptAgent:
    """
...
{previous_actions_str}"""
    ...
    def _parse_response_s2(
        ...
        def _regex_parse_tool_call(raw: str):
            """Regex-based fallback for malformed JSON tool calls (e.g. unescaped quotes)."""
            name_m = re.search(r'"name"\s*:\s*"([^"]+)"', raw)
            action_m = re.search(r'"action"\s*:\s*"([^"]+)"', raw)
            if not (name_m and action_m):
                return None
            result = {"name": name_m.group(1), "arguments": {"action": action_m.group(1)}}
            args = result["arguments"]

            coord_m = re.search(r'"coordinate"\s*:\s*\[\s*([\d.]+)\s*,\s*([\d.]+)\s*\]', raw)
            if coord_m:
                args["coordinate"] = [float(coord_m.group(1)), float(coord_m.group(2))]

            text_m = re.search(r'"text"\s*:\s*"(.*)"[^"]*\}\s*\}', raw, re.DOTALL)
            if text_m:
                args["text"] = text_m.group(1).replace('""', '"')

            keys_m = re.search(r'"keys"\s*:\s*\[([^\]]*)\]', raw)
            if keys_m:
                try:
                    args["keys"] = json.loads(f'[{keys_m.group(1)}]')
                except json.JSONDecodeError:
                    args["keys"] = [k.strip().strip('"\'') for k in keys_m.group(1).split(',')]

            for field in ("time", "pixels", "duration"):
                m = re.search(rf'"{field}"\s*:\s*([\d.]+)', raw)
                if m:
                    args[field] = float(m.group(1)) if '.' in m.group(1) else int(m.group(1))

            status_m = re.search(r'"status"\s*:\s*"([^"]*)"', raw)
            if status_m:
                args["status"] = status_m.group(1)
        ...
        def process_tool_call(json_str: str) -> None:
            ...
            try:
                ...
                logger.warning(f"json.loads failed ({e}), attempting regex fallback")
                tool_call = _regex_parse_tool_call(json_str)
                if tool_call is None:
                    logger.error(f"Regex fallback also failed for: {json_str[:200]}")
                    return
            ...
            try:
                ...
                logger.error(f"Missing key in parsed tool call: {e}")
    ...
    def call_llm(self, payload):
        ...
        # Use EvoCUA-scoped env vars only.
        base_url = os.environ.get("EVOCUA_BASE_URL", "url-xxx")
        api_key = os.environ.get("EVOCUA_API_KEY", "sk-xxx")
