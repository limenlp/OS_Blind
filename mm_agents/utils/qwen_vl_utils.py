...
def smart_resize(height, width, factor=28, min_pixels=56 * 56, max_pixels=14 * 14 * 4 * 1280, max_long_side=8192):
    ...
    2. The total pixel count stays within [min_pixels, max_pixels].
    3. The longest side does not exceed max_long_side.
    4. The aspect ratio stays approximately unchanged.
    """
...
def update_image_size_(image_ele: dict, min_tokens=1, max_tokens=12800, merge_base=2, patch_size=14):
    """Update image_ele size metadata based on min_tokens and max_tokens.
    ...
    Returns:
        Updated image_ele with the following additional key-value pairs:
        dict:
            - image_ele["resized_height"]: int actual input height to the model
            - image_ele["resized_width"]: int actual input width to the model
            - image_ele["seq_len"]: int sequence length consumed by the model input
    """
