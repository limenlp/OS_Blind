...
PROVIDER_TO_DEFAULT_MODEL_NAME: dict[(APIProvider, str), str] = {
    ...
    (APIProvider.VERTEX, "claude-opus-4-5"): "claude-opus-4-5@20251101",
    (APIProvider.ANTHROPIC, "claude-sonnet-4-5"): "claude-sonnet-4-5-20250929",
    (APIProvider.BEDROCK, "claude-sonnet-4-5"): "us.anthropic.claude-sonnet-4-5-20250929-v1:0",
    (APIProvider.VERTEX, "claude-sonnet-4-5"): "claude-sonnet-4-5-v1@20250929",
    (APIProvider.ANTHROPIC, "claude-sonnet-4-6"): "claude-sonnet-4-6",
    (APIProvider.BEDROCK, "claude-sonnet-4-6"): "anthropic.claude-sonnet-4-6",
    (APIProvider.VERTEX, "claude-sonnet-4-6"): "claude-sonnet-4-6",
}
    ...
    if response.content:
        for block in response.content:
            if isinstance(block, BetaTextBlock):
                ...
                tool_use_block = block.model_dump()
                if "caller" in tool_use_block:
                    del tool_use_block["caller"]
                res.append(cast(BetaToolUseBlockParam, tool_use_block))
        return res
    else:
        return []
