...
from mm_agents.s2.agents.grounding import ACI
from mm_agents.s2.core.module import BaseModule
from mm_agents.s2.core.knowledge import KnowledgeBase
from mm_agents.s2.memory.procedural_memory import PROCEDURAL_MEMORY
from mm_agents.s2.core.engine import OpenAIEmbeddingEngine
...
# Separate logger namespace by role so logs can be routed per-agent.
logger = logging.getLogger("desktopenv.agent.manager")
    ...
    def _generate_step_by_step_plan(
        ...
        if self.turn_count == 0:
            ...
            if self.search_engine is not None:
                _, retrieved_knowledge = self.knowledge_base.retrieve_knowledge(
