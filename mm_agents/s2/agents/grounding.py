...
from mm_agents.s2.memory.procedural_memory import PROCEDURAL_MEMORY
from mm_agents.s2.core.mllm import LMMAgent
from mm_agents.s2.utils.common_utils import (
    call_llm_safe,
...
class OSWorldACI(ACI):
    def __init__(
        ...
        self.coords1 = None
        self.coords2 = None
        # Trace of grounding calls for logging
        self.grounding_trace: List[Dict[str, Any]] = []
    ...
    def _get_grounding_target_size(self) -> Tuple[int, int]:
        # Keep target image size and coordinate scaling dimensions aligned.
        ...
        try:
            grounding_width = int(round(float(grounding_width)))
        except (TypeError, ValueError):
            grounding_width = 1000
        try:
            grounding_height = int(round(float(grounding_height)))
        except (TypeError, ValueError):
            grounding_height = 1000

        grounding_width = max(1, grounding_width)
        grounding_height = max(1, grounding_height)
        return grounding_width, grounding_height

    def _resize_screenshot_for_grounding(
        self, screenshot: Union[str, bytes, bytearray]
    ) -> Tuple[Union[str, bytes], Dict[str, Any]]:
        grounding_width, grounding_height = self._get_grounding_target_size()
        ...
        if isinstance(screenshot, str):
            with Image.open(screenshot) as image:
                ...
                if (original_width, original_height) != (
                    ...
                    resized = image.resize((grounding_width, grounding_height), Image.LANCZOS)
                    buffered = BytesIO()
            ...
            return screenshot, {
                "input_type": "path",
                "original_size": [grounding_width, grounding_height],
                "grounding_size": [grounding_width, grounding_height],
                "resized": False,
            }

        screenshot_bytes = (
            bytes(screenshot) if isinstance(screenshot, bytearray) else screenshot
        )
        with Image.open(BytesIO(screenshot_bytes)) as image:
            original_width, original_height = image.size
            if (original_width, original_height) == (grounding_width, grounding_height):
    ...
    def generate_coords(self, ref_expr: str, obs: Dict) -> List[int]:
        ...
        processed_screenshot, screenshot_meta = self._resize_screenshot_for_grounding(
            obs["screenshot"]
        )
        self.grounding_model.add_message(
            text_content=prompt, image_content=processed_screenshot, put_text_last=True
        )
        ...
        self._record_grounding_call(
            {
                "type": "grounder_coords",
                "query": ref_expr,
                "raw_response": response,
                "raw_coords": coords,
                "resized_coords": self.resize_coordinates(coords),
                "image_preprocess": screenshot_meta,
            }
        )
        return coords
    ...
    def assign_coordinates(self, plan: str, obs: Dict):
        ...
        # Reset trace for this action
        self.grounding_trace = []
    ...
    def _record_grounding_call(self, entry: Dict[str, Any]) -> None:
        self.grounding_trace.append(entry)

    def consume_grounding_trace(self) -> List[Dict[str, Any]]:
        trace = self.grounding_trace
        self.grounding_trace = []
        return trace
