...
class UITARSAgent:
    def __init__(
        ...
        **kwargs
        ...
        self.vlm = OpenAI(
            base_url=os.environ.get("DOUBAO_API_URL", "http://localhost:8002/v1"),
            api_key=os.environ.get("DOUBAO_API_KEY", "dummy"),
        )
    ...
    def predict(
        ...
        for turn, image in enumerate(self.history_images):
            ...
            if image.width * image.height > self.max_pixels:
                """
                If the image exceeds the pixel limit, compute a resize_factor
                so the total pixel count becomes less than or equal to
                max_pixels. The factor is derived via a square root so the
                aspect ratio is preserved and the original relative
                coordinates can still be reused directly.
                """
