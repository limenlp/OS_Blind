...
def config_list_from_json(
    ...
    # Process api_key_env field: convert it to api_key by reading from environment variable
    processed_config_list = []
    for config in config_list:
        processed_config = config.copy()
        if "api_key_env" in processed_config:
            api_key_env_var = processed_config.pop("api_key_env")
            api_key_value = os.environ.get(api_key_env_var)
            if api_key_value:
                processed_config["api_key"] = api_key_value
            # If api_key_env is present but the env var doesn't exist, we still remove api_key_env
            # and api_key will remain None (which may cause errors later, but that's expected behavior)
        processed_config_list.append(processed_config)

    return filter_config(processed_config_list, filter_dict)
