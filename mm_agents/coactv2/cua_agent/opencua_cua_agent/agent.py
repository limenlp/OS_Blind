...
class OpenCUAAgent:
    ...
    def call_llm(self, payload, model):
        ...
        for _ in range(20):
            response = httpx.post(
                f"http://10.3.87.39:8000/v1/chat/completions",
                json=payload,
                timeout=500,
                verify=False
            )
...
def run_opencua_cua(
    ...
    **_kwargs: Any,
    ...
    """
    Execute a GUI task with the OpenCUA agent and return the interaction history,
    ...
    agent = OpenCUAAgent(
        ...
        use_old_sys_prompt=use_old_sys_prompt,
        password=client_password or "osworld-public-evaluation",
    )
    agent.reset(logger)
    ...
    initial_path = os.path.join(save_path, "initial_screenshot.png")
    with open(initial_path, "wb") as file:
    ...
    for step_idx in range(1, max_steps + 1):
        try:
            response_text, actions, cot = agent.predict(
                instruction, {"screenshot": current_screenshot}, step_idx=step_idx
            )
        except Exception as exc:
            logger.error(f"OpenCUA agent predict failed at step {step_idx}: {exc}")
            status = "failure"
            final_reasoning = f"Agent failed at step {step_idx}: {exc}"
            break

        last_response = str(response_text)
        ...
        if isinstance(cot, dict):
            ...
            action_summary = cot.get("action")
            if thought:
                step_reason_parts.append(f"Thought: {thought}")
            if action_summary:
                step_reason_parts.append(f"Action: {action_summary}")
        if step_reason_parts:
            reasoning_log.append(f"Step {step_idx} - " + " ".join(step_reason_parts))

        if not actions:
            logger.warning("OpenCUA agent returned no actions.")
            status = "failure"
        ...
        for action_index, raw_action in enumerate(actions):
            ...
            if action_to_run == "WAIT":
                ...
                if wait_screenshot is not None:
                    ...
                    try:
                        ...
                        logger.warning(f"Failed to persist WAIT screenshot: {exc}")
                    history_inputs.append(
                        {
                            "role": "user",
                            "content": [
                                {"type": "text", "text": f"Step {step_idx}: WAIT action executed."},
                                {
                                    "type": "image_base64",
            ...
            if action_to_run == "DONE":
                ...
                final_reasoning = (
                    cot.get("action")
                    if isinstance(cot, dict) and cot.get("action")
                    else "Agent indicated the task is complete."
                )
                terminated = True
                break
            ...
            if action_to_run == "FAIL":
                ...
                failure_reason = (
                    cot.get("action")
                    if isinstance(cot, dict) and cot.get("action")
                    else "Agent reported it cannot complete the task."
                )
                final_reasoning = failure_reason
                terminated = True
                break

            logger.info(f"OpenCUA action at step {step_idx}: {action_to_run}")
            try:
                obs_dict, *_ = env.step(action_to_run, sleep_after_execution)
            except Exception as exc:
                logger.error(f"env.step failed for OpenCUA action: {exc}")
                status = "failure"
            ...
            if isinstance(obs_dict, dict) and obs_dict.get("screenshot") is not None:
                ...
                try:
                    ...
                    logger.warning(f"Failed to persist screenshot for step {step_idx}: {exc}")
                        ...
                        "content": [
                            {
                                ...
                                "text": (
                                    ...
                                    f"{transform_agnet_action_to_code_block(action_to_run)}"
                                ),
                            },
                            {
                                "type": "image_base64",
    ...
    if not final_reasoning:
        final_reasoning = last_response or "No final reasoning provided by the agent."
    ...
    message_body = "\n\n".join(line.strip() for line in message_lines if line and line.strip())
