...
def run_single_example(agent, env, example, max_steps, instruction, args, example_result_dir, scores):
    ...
    # Set result_dir for AttackableDesktopEnv (if available)
    # This enables attack artifact saving to the correct directory
    ...
    if not getattr(env, "session_recording_active", False):
        try:
            env.controller.start_recording()
            recording_started = True
        except Exception as record_err:
            logger.warning("Failed to start recording for %s: %s", example.get("id"), record_err)
    try:
        while not done and step_idx < max_steps:
            ...
            for action in actions:
                ...
                # Add attack status if available (for AttackableDesktopEnv)
                if hasattr(env, 'get_attack_status'):
                    traj_entry["attack_status"] = env.get_attack_status()
        ...
        if recording_started:
            try:
                ...
                logger.warning("Failed to end recording for %s: %s", example.get("id"), record_err)
...
def run_single_example_opencua(agent, env, example, max_steps, instruction, args, example_result_dir, scores):
    ...
    # (attack_generation_log.json, attack_log.jsonl) are saved correctly
    if hasattr(env, 'set_result_dir'):
        env.set_result_dir(example_result_dir)
    ...
    while not done and step_idx < max_steps:
        ...
        for action in actions:
            ...
            # Add attack status if available (for AttackableDesktopEnv)
            if hasattr(env, 'get_attack_status'):
                log_entry["attack_status"] = env.get_attack_status()
...
def run_single_example_evocua(agent, env, example, max_steps, instruction, args, example_result_dir, scores):
    ...
    # Optional: log task completion if lib_results_logger is available
    try:
        from lib_results_logger import log_task_completion
        log_task_completion(example, result, example_result_dir, args)
    except ImportError:
        pass  # lib_results_logger is optional
