...
cwd = os.getcwd()
if cwd not in sys.path and cwd != script_dir:
    sys.path.append(cwd)

from lib_env import load_env
load_env(__file__, "coactv2", override=True)

from multiprocessing import Pool, cpu_count
from functools import partial
from typing import Dict, List
from mm_agents.coactv2.orchestrator_agent import OrchestratorAgent, OrchestratorUserProxyAgent
from mm_agents.coactv2.autogen import LLMConfig
from mm_agents.coactv2.coact_prompt import get_task_description, get_task_description_cua_only, TASK_DESCRIPTION_CODING_ONLY
...
def config() -> argparse.Namespace:
    ...
    # agent config
    parser.add_argument("--mode", type=str, default="coact_cua_only", choices=["human", "hybrid", "coact_cua_only", "coact_coding_only", "coact_opensource_sft"])
    parser.add_argument("--oai_config_path", type=str, default="mm_agents/coactv2/OAI_CONFIG_LIST")
    ...
    parser.add_argument("--cua_model", type=str, default="claude-sonnet-4-5-20250929")
    parser.add_argument("--orchestrator_max_steps", type=int, default=6)
    parser.add_argument("--coding_max_steps", type=int, default=5)
    parser.add_argument("--cua_max_steps", type=int, default=5)
    parser.add_argument("--cut_off_steps", type=int, default=30)
    ...
    parser.add_argument(
        "--test_config_base_dir", type=str, default="evaluation_example_safety/examples"
    )

    # logging related
    parser.add_argument("--result_dir", type=str, default="./results_coactv2_gpt5_claude45_round1")
            ...
            with llm_config:
                if mode == "coact_cua_only":
                    orchestrator = OrchestratorAgent(
                        name="orchestrator",
                        system_message=get_task_description_cua_only(cua_max_steps),
                        mode=mode
                    )
                    ...
                    orchestrator = OrchestratorAgent(
                        name="orchestrator",
                        system_message=get_task_description(cua_max_steps),
                        mode=mode
                    )
...
if __name__ == "__main__":
    ...
    with open(os.path.join(args.result_dir, f'coact_{args.mode}', f'orchestrator_system_prompt.txt'), "w") as f:
        f.write(get_task_description(args.cua_max_steps))
    ...
    def print_popup_asr(result_dir, mode):
        """Read attack_log.jsonl from 12_pop_ups_attack and print average ASR."""
        popup_dir = os.path.join(result_dir, f'coact_{mode}', '12_pop_ups_attack')
        if not os.path.exists(popup_dir):
            print("[12_pop_ups_attack] Directory not found, skipping ASR calculation.")
            return
        asr_list = []
        for task_id in os.listdir(popup_dir):
            log_path = os.path.join(popup_dir, task_id, 'attack_log.jsonl')
            ...
            try:
                ...
                if lines:
                    ...
                    asr_list.append(1 if last.get('attack_success') else 0)
            except Exception as e:
                print(f"[12_pop_ups_attack] Failed to read {task_id}: {e}")
        if asr_list:
            print(f"[12_pop_ups_attack] Avg ASR: {sum(asr_list)/len(asr_list)*100:.1f}% ({sum(asr_list)}/{len(asr_list)} tasks attacked)")
        else:
            print("[12_pop_ups_attack] No attack_log.jsonl found, cannot compute ASR.")
    ...
    if not tasks:
        ...
        print_popup_asr(args.result_dir, args.mode)
    else:
