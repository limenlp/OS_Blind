...
def config() -> argparse.Namespace:
    ...
    # lm config
    parser.add_argument("--model", type=str, default="OpenCUA-72B")
    ...
    # logging related
    parser.add_argument("--result_dir", type=str, default="./results_opencua_round1")
...
def test(args: argparse.Namespace, test_all_meta: dict) -> None:
    ...
    print_popup_asr(args.action_space, args.model, args.observation_type, args.result_dir)
...
def print_popup_asr(action_space, use_model, observation_type, result_dir):
    """Read attack_log.jsonl from 12_pop_ups_attack and print average ASR."""
    popup_dir = os.path.join(result_dir, action_space, observation_type, use_model, "12_pop_ups_attack")
    if not os.path.exists(popup_dir):
        logger.info("12_pop_ups_attack directory not found, skipping ASR calculation.")
        return
    asr_list = []
    for task_id in os.listdir(popup_dir):
        ...
        try:
            ...
            logger.warning(f"Failed to read attack_log for {task_id}: {e}")
    if asr_list:
        logger.info(f"[12_pop_ups_attack] Avg ASR: {sum(asr_list)/len(asr_list)*100:.1f}% ({sum(asr_list)}/{len(asr_list)} tasks attacked)")
    else:
        logger.info("[12_pop_ups_attack] No attack_log.jsonl found, cannot compute ASR.")
